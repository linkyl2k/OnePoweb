...
def index():
    ...
    if opt_weekday:
        try:
            ...
            days_order = ["שבת", "שישי", "חמישי", "רביעי", "שלישי", "שני", "ראשון"]  # Обратный порядок: от субботы к воскресенью
            ...
            if by_wd.empty:
                ...
                # Порядок дней на иврите (обратный: от субботы к воскресенью)
                order_he = ["שבת", "שישי", "חמישי", "רביעי", "שלישי", "שני", "ראשון"]
                # Маппинг дней недели на разные языки (обратный порядок: от субботы к воскресенью)
                day_mapping = {
                    "he": {"שבת": "שבת", "שישי": "שישי", "חמישי": "חמישי", "רביעי": "רביעי", "שלישי": "שלישי", "שני": "שני", "ראשון": "ראשון"},
                    "en": {"שבת": "Saturday", "שישי": "Friday", "חמישי": "Thursday", "רביעי": "Wednesday", "שלישי": "Tuesday", "שני": "Monday", "ראשון": "Sunday"},
                    "ru": {"שבת": "Суббота", "שישי": "Пятница", "חמישי": "Четверг", "רביעי": "Среда", "שלישי": "Вторник", "שני": "Понедельник", "ראשון": "Воскресенье"}
                }
    ...
    if opt_heatmap:
        try:
            ...
            if HOUR_COL in df.columns and "יום בשבוע" in df.columns:
                ...
                # סידור ימים בסדר נכון (обратный порядок: от субботы к воскресенью)
                days_order = ["שבת", "שישי", "חמישי", "רביעי", "שלישי", "שני", "ראשון"]
